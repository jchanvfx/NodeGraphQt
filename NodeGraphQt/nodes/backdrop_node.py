...
class BackdropNodeItem(AbstractNodeItem):
    ...
    def itemChange(self, change, value):
        # Handle view transform changes to trigger re-rendering
        if change == QtWidgets.QGraphicsItem.ItemSceneHasChanged:
            self.update()
        elif change == QtWidgets.QGraphicsItem.ItemTransformHasChanged:
            self.update()
        return super(BackdropNodeItem, self).itemChange(change, value)
    ...
    def on_sizer_pos_mouse_release(self):
        ...
        # Force update after resize completion
        self.update()
    ...
    def on_sizer_double_clicked(self):
        ...
        # Force update after auto-resize
        self.update()
    ...
    def paint(self, painter, option, widget):
        ...
        # Check if view transform has changed and force update if needed
        if self.scene() and self.scene().views():
            current_transform = self.scene().views()[0].transform()
    ...
    def from_dict(self, node_dict):
        ...
        # Force update after loading from dict
        self.update()
