...
from NodeGraphQt.node_qgraphics.node_graphics_item import NodeGraphicsItem
from NodeGraphQt.node_qgraphics.port_graphics_item import PortGraphicsItem, CustomPortGraphicsItem
...
class BaseNodeGraphicsItem(NodeGraphicsItem):
    """
    Base Node item.
    ...
    def mouseDoubleClickEvent(self, event):
        """
        Re-implemented to emit "node_double_clicked" signal and to toggle
        the text item in edit mode.
    ...
    def itemChange(self, change, value):
        ...
        if change == QtWidgets.QGraphicsItem.ItemSelectedChange and self.scene():
            ...
            if not self.get_property('selected'):
    ...
    def setToolTip(self, tooltip):
        """
        Re-implemented so we pre-append additional info.

        Args:
            tooltip (str): tool tip text.
        """
        prepend_info = '<b>{}</b>'.format(self.get_property('name'))
        disabled = self.get_property('disabled')
        if disabled:
            prepend_info += (
                '<br/><font color="red"><b>({})</b></font>'
                .format(self._x_item.text)
            )
        prepend_info += '<br/>{}<br/>'.format(self.get_property('type_'))
        tooltip = prepend_info + tooltip
        super().setToolTip(tooltip)

    def paint(self, painter, option, widget):
        """
        Draws the node base and Not the ports.
    ...
    def set_property(self, name, value):
        ...
        if name == 'text_color':
            self.text_item().setDefaultTextColor(QtGui.QColor(*value))
        elif name == 'icon':
            if not value:
    ...
    def paint_horizontal(self, painter):
        ...
        Re-implement this for custom node drawing.
        ...
        # node selected colors
        n_sel_color = NodeEnum.SELECTED_COLOR.value
        n_sel_bdr_color = NodeEnum.SELECTED_BORDER_COLOR.value
        ...
        margin = 1.0
        rect = QtCore.QRectF(n_rect.left() + margin,
                             n_rect.top() + margin,
                             n_rect.width() - (margin * 2),
                             n_rect.height() - (margin * 2))

        radius = 4.0
        painter.setBrush(QtGui.QColor(*n_color))
        painter.drawRoundedRect(rect, radius, radius)

        # draw overlay selection color on background when selected.
        if n_selected:
            painter.setBrush(QtGui.QColor(*n_sel_color))
            painter.drawRoundedRect(rect, radius, radius)
        ...
        if n_selected:
            painter.setBrush(QtGui.QColor(*n_sel_bdr_color))
        else:
        ...
        if n_selected:
            ...
            border_width = 0.8
            border_color = QtGui.QColor(*n_border_color)
    ...
    def paint_vertical(self, painter):
        ...
        if n_selected:
            border_width = 1.2
            border_color = QtGui.QColor(*n_sel_bdr_color)
        border_rect = QtCore.QRectF(rect.left(), rect.top(),
                                    rect.width(), rect.height())
    ...
    def add_port(self, port):
        ...
        """
        text = QtWidgets.QGraphicsTextItem(port.label, self)
        text.font().setPointSize(8)
        text.setFont(text.font())
        text.setVisible(port.display_name)
        text.setCacheMode(self.cacheMode())
        if port.port_type == PortTypeEnum.IN.value:
            self._in_ports[port.name] = port
            self._in_port_txt_items[port.name] = text
    ...
    def add_port_input(self, name='input', multi_port=False, display_name=True,
        ...
        Returns:
            PortGraphicsItem: input port qgraphics item.
        """
        if painter_func:
            port = CustomPortGraphicsItem(self, painter_func)
        else:
            port = PortGraphicsItem(self)
        port.name = name
    ...
    def add_port_output(self, name='output', multi_port=False, display_name=True,
                        locked=False, painter_func=None):
        """
        Adds a port qgraphics item into the node with the "port_type" set as
        ``PortTypeEnum.OUT``.
        ...
        Returns:
            PortGraphicsItem: output port qgraphics item.
        """
        if painter_func:
            port = CustomPortGraphicsItem(self, painter_func)
        else:
    ...
    def remove_port(self, port):
        ...
        Args:
            port (PortGraphicsItem): port item
        """
        port_name = port.name()
        if port.port_type == PortTypeEnum.IN:
            self._in_ports.pop(port_name)
            text_item = self._in_port_txt_items.pop(port_name)
    ...
    def _set_base_size(self, add_w=0.0, add_h=0.0):
        ...
        """
        calc_width, calc_height = self.calc_size(add_w, add_h)
        if calc_width < NodeEnum.WIDTH.value:
            calc_width = NodeEnum.WIDTH.value
        if calc_height < NodeEnum.HEIGHT.value:
            calc_height = NodeEnum.HEIGHT.value
        self.set_property('width', calc_width)
        self.set_property('height', calc_height)
